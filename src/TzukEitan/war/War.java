...
public class War extends Thread {
	...
	public War() {
		...
		ApplicationContext theContext = new ClassPathXmlApplicationContext(WarDb.class.getResource("bean.xml").toExternalForm());
		if (Constans.JPA){
			warDb = (WarDb)theContext.getBean("WarDbJPA");
		} else{
			warDb = (WarDb)theContext.getBean("WarDbJDBC");
		}
		warDb.startWar(Constans.JPA);
		
		//theContext = new ClassPathXmlApplicationContext(WarDb.class.getResource("logger.xml").toExternalForm());
		
		//WarLogger logger = (WarLogger)theContext.getBean("WarLogger");
		
		//registerListenerts(new WarLogger());
		//registerListenerts(logger);
	...
	public void run() {
		...
		//WarLoggerA.closeAllHandlers();
		//WarLoggerA.closeWarHandler();
		//warHandler.close();
	}// run
			...
			if (el.getLauncherId().equals(launcherId) && el.isAlive()) {
				...
				if (el.getCurrentMissile() == null) {
					synchronized (el) {
						...
						warDb.launchEnemyMissile(ret, launcherId, destination, damage, flyTime);
						el.notify();
						return ret;
					}//synchronized
